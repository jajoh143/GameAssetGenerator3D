"""Top-level Blender script: generate a floor asset.

Usage:
    blender --background --python scripts/generate_floor.py -- [options]

Options:
    --output PATH        Output file (default: assets/floor.glb)
    --format FORMAT      Export format: glb, gltf, fbx, obj
    --variation NAME     Floor type: concrete, metal_plate, wood_plank, tile, asphalt, cobblestone
    --width FLOAT        Floor width in meters (default: 4.0)
    --length FLOAT       Floor length in meters (default: 4.0)
    --theme NAME         Style theme: modern, fantasy, industrial, medieval
    --material NAME      Material override: brick, concrete, metal, wood, stone, tile
    --wear FLOAT         Wear level 0.0-1.0 (default: 0.6)
"""

import sys
import os
import argparse

project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if project_root not in sys.path:
    sys.path.insert(0, project_root)

from generators.floor import generate
from generators.style import AssetStyle
from generator.export import export


def parse_args():
    argv = sys.argv
    if "--" in argv:
        argv = argv[argv.index("--") + 1:]
    else:
        argv = []

    parser = argparse.ArgumentParser(description="Generate a floor asset")
    parser.add_argument("--output", default="assets/floor.glb")
    parser.add_argument("--format", default=None)
    parser.add_argument("--variation", default="concrete")
    parser.add_argument("--width", type=float, default=4.0)
    parser.add_argument("--length", type=float, default=4.0)
    parser.add_argument("--theme", default="modern")
    parser.add_argument("--material", default=None)
    parser.add_argument("--wear", type=float, default=0.6)
    return parser.parse_args(argv)


def main():
    args = parse_args()

    config = {
        "width": args.width,
        "length": args.length,
        "depth": 0.1,
        "variation": args.variation,
    }

    style_kwargs = {"theme": args.theme, "wear": args.wear}
    if args.material:
        style_kwargs["material"] = args.material
    else:
        mat_map = {
            "concrete": "concrete", "metal_plate": "metal", "wood_plank": "wood",
            "tile": "tile", "asphalt": "concrete", "cobblestone": "stone",
        }
        style_kwargs["material"] = mat_map.get(args.variation, "concrete")

    style = AssetStyle(**style_kwargs)

    print(f"Generating floor: variation={args.variation}, style={style}")
    generate(config, style)

    output = os.path.abspath(args.output)
    export(output, args.format)
    print(f"Done! Asset saved to: {output}")


if __name__ == "__main__":
    main()
